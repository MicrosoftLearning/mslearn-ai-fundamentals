...
Azure Content Understanding is a Foundry service that uses generative AI to turn unstructured, multimodal content (documents, images, video, audio) into structured, usable outputs like JSON. It processes content by extracting, classifying, and generating fields with confidence scores and source grounding.
...
1. In a web browser, open [Microsoft Foundry](https://ai.azure.com){:target="_blank"} at `https://ai.azure.com`  and sign in using your Azure credentials. Close any tips or quick start panes that are opened the first time you sign in, and if necessary use the **Foundry** logo at the top left to navigate to the home page (close the **Help** pane if it's open).
...
1. In the *new* Foundry portal, navigate to the banner at the top of the screen and select **Build**.

    ![Screenshot of the new Foundry's top banner with Build selected.](./media/new-foundry-build-tab-1.png)

1. On the *Build* page, navigate to the menu on the left-side of the screen (you may need to expand it by clicking on the expand icon at the bottom of the menu). From the left-side menu, select **Models**. Then, at the top of the *Models* page, select **AI Services**.
...
1. Identify the services that begin with **Content Understanding**: 
...
1. Select **Content Understanding - Read**. The *Read* capability is the first step in content understanding—it reads and extracts text, but doesn’t try to understand structure or meaning yet. Let's see it in action.

1. Select the sample **read_barcode.pdf** and use the **Run analysis** button to extract information from the document. When analysis is complete, view the results.
...
#### Try out Content Understanding's full analyzer capabilities 

1. From the *Build - Models* page and *AI Services* tab, select **Content Understanding**. A full analyzer turns content into structured data you can use in apps or workflows.
...
7. In the pane on the right where the extracted fields are displayed, view the **Result** tab to see the raw results in JSON. 
...
| Capability | Read | Layout | Full Analyzer (Content Understanding) |
|------------|------|--------|--------------------------------------|
| **Core focus** | Extract text | Understand structure | Extract + reason + generate |
| **Text extraction (OCR)** | ✅ | ✅ | ✅ |
| **Structure detection (tables, sections)** | ❌ | ✅ | ✅ |
| **Layout relationships (hierarchy, positions)** | ❌ | ✅ | ✅ |
| **Field extraction (schema-based)** | ❌ | ❌ | ✅ |
| **Classification / routing** | ❌ | ❌ | ✅ |
| **Generative insights (summaries, inferred fields)** | ❌ | ❌ | ✅ |
| **Output type** | Plain text | Structured (Markdown + layout metadata) | Fully structured JSON + generated fields |
